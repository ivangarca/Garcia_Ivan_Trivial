...
class TrivialViewModel : ViewModel() {
    ...
    //creo la lista de las preguntas  con sus respuestas
    ...
    val uiState = _uiState.asStateFlow() //lee la vista del ViewTrivial

    fun comprovarResposta(idResposta: Int) {
        val estatActual = _uiState.value

        if (!estatActual.jocFinalitzat) { // si el juego no ha acabado hace lo del if

            val esCorrecta = idResposta == estatActual.preguntaActual?.respostaCorrecta //miro si coincide el parametro del metodo con la respuesta correcta de la pregunta

            var nousPuntsP1 = estatActual.puntsP1 //puntos jugador 1
